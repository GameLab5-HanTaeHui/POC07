...
### v0.26 — 보스 기사형 코드 구현 1차 (구조 설계 + 핵심 컴포넌트)

**작업 내용**
보스 기사형 전체 아키텍처 확정 및 핵심 컴포넌트 1차 구현.
...
| `BossKnightAI.cs` | v1.0 | 보스 전용 AI. 10상태 + 회피 기동 + Counter |
| `BossKnight.cs` | v1.0 | 보스 루트. Phase 전환 + 무적 + TakeDamage override |
...
EnemyAI vs BossKnightAI 차이
  EnemyAI    : 5상태 / 단일 패턴 / Patrol 있음 / EnemySensor 의존
  BossKnightAI: 10상태 / 다수 패턴 / Patrol 없음 / 항상 플레이어 인식
                Pause/Resume / Counter 상태 / Dodge 상태

BossPatternBase vs EnemyAttackBase 차이
  EnemyAttackBase: Execute 1단계
  BossPatternBase: Warning / Active / Recovery 3단계
                   Pause / Resume / Interrupt / SetSpeedMultiplier
                   OnSealHit() → BossPatternSealResult 반환

패턴 속도 배율 (_speedMultiplier)
  팔 봉인 시 BossPartComponent.ApplySpeedMultiplier() 호출
  → 연결된 패턴의 WaitScaled() 시간 배율 증가
  → 플레이어 공략 시간 자동 확보
```
